...
case class ListTopics(topics: Seq[String], nextToken: Option[String])

case class Subscription(topicArn: String, subscriptionArn: String, owner: String, endpoint: Endpoint)

case class SubscriptionList(subscriptions: Seq[Subscription], nextToken: Option[String])

case class TopicAttributes(
...
case class SubscriptionAttributes(
